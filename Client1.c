...
/**
 * Эта функция создает сокет и подключается к серверу, а затем ожидает ввода данных пользователем для
 * отправки сообщений на сервер или выполнения определенных действий.
 * 
 * return void, что означает, что он не возвращает никакого значения.
 */
void manage_peer()
{
	/* Приведенный выше код реализует клиентскую программу сокетов на языке C. Он создает сокет,
	подключается к серверу с указанным IP-адресом и номером порта, а затем входит в цикл для отправки
	сообщений на сервер. Пользователь может вводить различные команды, такие как «IndexGet»,
	«FileHash», «FileDownload» и «FileUpload», чтобы запрашивать различные действия с сервера.
	Программа также обрабатывает команду «Q» или «q», чтобы закрыть соединение и выйти из программы. */
	int sockfd, portno, n;
...
/**
 * Функция реализует сервер, который прослушивает входящие клиентские соединения и обрабатывает
 * различные команды, отправленные клиентом.
 * 
 * return Функция ничего не возвращает, так как возвращаемый тип недействителен.
 */
void manage_host()
{
	/* Приведенный выше код реализует сервер, который прослушивает входящие клиентские подключения и
	обрабатывает различные команды, отправленные клиентом. Он создает сокет, привязывает его к порту,
	прослушивает входящие соединения и принимает соединение от клиента. Затем он входит в цикл, где он
	считывает команды от клиента и выполняет соответствующие действия на основе команды. Поддерживаемые
	команды: «IndexGet», «FileHash», «FileDownload» и «FileUpload». Если команда не распознана, она
	сохраняет сообщение в массиве для последующей обработки. Цикл продолжается до тех пор, пока клиент
	не отправит "q" или " */
	...
	servi_addr.sin_port = htons(SERV_PORT);
...
/**
 * Функция Indexget выполняет различные операции на основе введенной команды и отправляет результат
 * клиенту.
 * 
 * param read_buffer Массив символов, содержащий сообщение, полученное от клиента.
 * param newsockfd Дескриптор файла для соединения сокета с клиентом.
 */
...
/**
 * Функция cIndexget получает данные из сокета и выводит информацию о файле в зависимости от типа
 * полученного запроса.
 * 
 * param write_buffer Массив символов, содержащий сообщение для отправки через сокет.
 * param sockfd Дескриптор файла для подключения к сокету.
 */
...
/**
 * Функция Filehash проверяет, является ли входная команда «Verify» или «CheckAll», и выполняет
 * хеширование либо одного файла, либо всех файлов в каталоге соответственно.
 * 
 * param read_buffer Массив символов, содержащий сообщение, полученное от клиента.
 * param newsockfd Параметр «newsockfd» — это указатель на целое число, представляющее файловый
 * дескриптор сокета для текущего соединения.
 */
...
/**
 * Функция выполняет хеширование заданного файла и отправляет результат в указанный сокет.
 * 
 * param l1 Массив символов, содержащий имя хешируемого файла.
 * param newsockfd Параметр «newsockfd» представляет собой указатель на целое число, которое
 * используется для ссылки на файловый дескриптор сокета для вновь принятого клиентского соединения.
 */
...
/**
 * Функция загружает файл с сервера и сохраняет его на стороне клиента.
 * 
 * param write_buffer Массив символов, содержащий сообщение для отправки на сервер.
 * param sockfd Дескриптор файла сокета, используемый для связи между клиентом и сервером.
 * 
 * return У функции нет оператора return, поэтому она ничего не возвращает.
 */
...
/**
 * Функция загружает файл с сервера и отправляет клиенту его метку времени, размер, имя и хэш.
 * 
 * param read_buffer Массив символов, содержащий сообщение, полученное от клиента.
 * param newsockfd Дескриптор файла для соединения сокета с клиентом.
 * 
 * return Функция не имеет возвращаемого типа, поэтому ничего не возвращает.
 */
...
/**
 * Функция загружает файл на сервер с помощью соединения через сокет и отправляет на сервер размер и
 * имя файла.
 * 
 * param write_buffer Массив символов, содержащий сообщение для отправки на сервер.
 * param sockfd Дескриптор файла сокета, используемый для связи между клиентом и сервером.
 * 
 * return В данном коде нет оператора возврата, поэтому ничего не возвращается.
 */
...
/**
 * Функция позволяет серверу получить файл от клиента, проверить, разрешено ли его загружать, и
 * сохранить его в каталоге сервера.
 * 
 * param read_buffer Массив символов, используемый для чтения данных из сокета.
 * param newsockfd Дескриптор файла для соединения сокета с клиентом.
 * 
 * return Функция не имеет возвращаемого типа, поэтому ничего не возвращает.
 */
